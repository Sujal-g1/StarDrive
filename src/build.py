...
def build_hlines(self):
    ...
    with self.canvas:
        ...
        for _ in range(self.hlines_number):
...
def build_tiles(self):
    ...
    with self.canvas:
        ...
        for _ in range(self.tiles_number_started):
...
def generate_tiles_coordinates(self):
    ...
    for i in range(
        len(self.tiles_coordinates) - 1,
        -1,
        -1
    ):

        if (
            self.tiles_coordinates[i][1]
            < self.current_loop
        ):
    ...
    self.tiles_number = (
        self.tiles_number_started
        if self.state_game_started
        else self.tiles_number_not_started
    )

    for _ in range(
        len(self.tiles_coordinates),
        self.tiles_number
    ):
...
def get_tile_coordinates(
    ...
    y = self.get_lineh_from_index(
        t_y - self.current_loop
    )
